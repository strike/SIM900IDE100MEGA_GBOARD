...
int SIMCOM900::configandwait(char* pin)
{
  ...
  {  	
	...
	//while (_tf.find("+CGREG: 0,"))  // CHANGE!!!!
	{
		...
		SimpleWriteln_P(F("AT+CMGF=1")); //SMS text mode.
		delay(200);
		  // Buah, we should take this to readCall()
		SimpleWriteln_P(F("AT+CLIP=1")); //SMS text mode.
		delay(200);
 ...
 int SIMCOM900::readCellData(int &mcc, int &mnc, long &lac, long &cellid)
{
  if (getStatus()==IDLE)
   ...
   //_cell.flush();
  SimpleWriteln_P(F("AT+QENG=1,0")); 
  SimpleWriteln_P(F("AT+QENG?")); 
...
boolean SIMCOM900::readSMS(char* msg, int msglength, char* number, int nlength)
{
  ...
  */
  //_tf.setTimeout(_GSM_DATA_TOUT_);
  //_cell.flush();
  SimpleWriteln_P(F("AT+CMGL=\"REC UNREAD\",1"));
...
void SIMCOM900::SimpleWrite_P(const __FlashStringHelper *comm)
{
  _cell.write_P(comm);
}
...
void SIMCOM900::SimpleWriteln_P(const __FlashStringHelper *comm)
{
...
void GSM::SetSpeaker(byte off_on)
{
  ...
  if (off_on) {
    //SendATCmdWaitResp(F("AT#GPIO=5,1,2"), 500, 50, "#GPIO:", 1);
  }
  else {
    //SendATCmdWaitResp(F("AT#GPIO=5,0,2"), 500, 50, "#GPIO:", 1);
  }
...
byte GSM::IsUserButtonPushed(void)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  //if (AT_RESP_OK == SendATCmdWaitResp(F("AT#GPIO=9,2"), 500, 50, "#GPIO: 0,0", 1)) {
