...
char SMSGSM::IsSMSPresent(byte required_status) 
{
  ...
  switch (required_status) {
    case SMS_UNREAD:
      gsm.SimpleWriteln_P(F("AT+CMGL=\"REC UNREAD\""));
      break;
    case SMS_READ:
      gsm.SimpleWriteln_P(F("AT+CMGL=\"REC READ\""));
      break;
    case SMS_ALL:
      gsm.SimpleWriteln_P(F("AT+CMGL=\"ALL\""));
      break;
  }
